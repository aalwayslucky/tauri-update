...
tauri_releases_bp = Blueprint('tauri_releases', __name__, url_prefix='/tauri-releases',
                              template_folder='blueprints/tauri_releases/templates')

GOOGLE_KEEP_DESKTOP_REPO = 'aalwayslucky/NimbusTrading'
            ...
            if asset['name'].endswith(extension):
                for platform in for_platforms:
                    release_response['platforms'][platform] = {
                        **release_response['platforms'].get(platform, {}), 'url': asset['browser_download_url']}
                ...
                for platform in for_platforms:
                    release_response['platforms'][platform] = {
                        **release_response['platforms'].get(platform, {}), 'signature': sig}
    return release_response


@tauri_releases_bp.route('/NimbusTrading/<platform>/<current_version>')
def google_keep_desktop_api(platform, current_version):
    ...
    try:
        ...
        latest_maj, latest_min, latest_patch = latest_version.lstrip(
            'v').split('.')
...
@tauri_releases_bp.route('/NimbusTrading/')
